...
namespace BankAccountKataTDD
{
    ...
    public  class BankAccount : IBankAccount
    {
        ...
        {
            CheckDepositAmountIsPositive(amount);
            IncreaseBalance(amount);
            SaveTransaction(amount);

        }

        private static void CheckDepositAmountIsPositive(int amount)
        {
            if (amount < 0)
        ...
        {
            CheckWithdrawAmountIsValide(amount);
            DecreaseBalance(amount);
            SaveTransaction(-amount);
        }

        private void CheckWithdrawAmountIsValide(int amount)
        {
        ...
        public IEnumerable<Transaction> GetTransactions() { // to avoid any changes in the list
